      ...
      window.addEventListener('DOMContentLoaded', event => {
        ...
        document.querySelector('#chat-message-input').onkeyup = function(e) {
            if (e.keyCode === 13) {  // on releasing enter, return key
        ...
        document.querySelector('#chat-message-input').onpaste = function(e) {
            ...
            if (item.type.includes('image')) {
                let blob = item.getAsFile();//bi nary large object(image)
        ...
        function loadMessage() {
            fetch("{% url 'chat:history' chatgroup.id %}")
                ...
                .then( data => {
                    for (let msg of data) {
                      if (msg.image_caption == null) {
                        $.ajax({
                          type: 'GET',
                          url: "{% url 'chat:get'  %}",
                          data: { message: msg.message, nonce:msg.nonce},
                          contentType: "application/json; charset=utf-8",
                          success: function(data){
                            // alert(data)?
                             broadcastMessage(data.message, msg.username, msg.message_type, msg.image_caption, msg.date_created)
        ...
        document.querySelector('#OpenImgUpload').onclick = function(e) {
          $('#imgupload').trigger('click');
            let item = document.getElementsByName("file").items[0];
            if (item.type.includes('image')) {
